<template>
	<view class="wrap">
		<view class="top-view">
			<view class="u-search-box"><u-search :show-action="false" v-model="keyword" input-align="center" placeholder="搜索地块" @search="reload"></u-search></view>
		</view>
		<view class="scroll-box">
			<scroll-view scroll-y class="scroll-view" @scrolltolower="reachBottom">
				<view class="page-box">
					<u-swipe-action :show="item.show" :index="index" v-for="(item, index) in activityData" :key="item.id" @click="click" @open="open" :options="options">
		...
		<view class="round-click">
			<navigator url="/pages/subLand/map/main"><u-icon name="plus" size="35" color="#fff"></u-icon></navigator>
		</view>
	</view>
</template>
...
export default {
	...
	methods: {
		...
		async getActivityData(initList = false, nowPageSize) {
			...
			initList ? (this.activityData = tranList(res.data)) : (this.activityData = this.activityData.concat(tranList(res.data)));
