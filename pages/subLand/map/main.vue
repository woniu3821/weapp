		...
		<view class="content">
			<u-form :model="form" ref="uForm" label-width="150">
				<u-form-item label="中心坐标">
					<u-input disabled :value="pointText" />
				</u-form-item>
				<u-form-item label="农田面积" prop="farmlandArea">
					<u-input disabled :value="form.farmlandArea" />
				</u-form-item>
				<u-form-item label="农田名称" prop="name">
	...
	function transPointsToBD(data) {
		return data.map(item => {
			const {
				longitude,
				latitude
			} = QQMapTobMap(item.longitude, item.latitude);
			return {
				lng: longitude,
				lat: latitude
			};
